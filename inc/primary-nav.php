<ul class="nav nav-pills">
  <li><a href="http://localhost:8888/wp/services/">Services</a></li>
  <li><a href="#" class="dropdown-toggle" data-toggle="dropdown">Work <span class="caret"></a>
    <ul class="dropdown-menu" role="menu">
      <li><a href="#">Flagship Programs</a></li>
      <li><a href="http://localhost:8888/wp/client-projects/">Client Projects</a></li>
      <li><a href="http://localhost:8888/wp/white-papers/">White Papers</a></li>
    </ul>
  </li>
  <li><a href="#" class="dropdown-toggle" data-toggle="dropdown">About <span class="caret"></a>
    <ul class="dropdown-menu" role="menu">
      <li><a href="http://localhost:8888/wp/team/">Team</a></li>
      <li><a href="#">Board</a></li>
      <li><a href="#">Partners</a></li>
      <li><a href="#">Program Sites</a></li>
      <li><a href="#">Funders</a></li>
      <li><a href="#">In-Kind Donors</a></li>
    </ul>
  </li>
  <li><a href="http://localhost:8888/wp/press/">Press</a></li>
  <li><a href="#">Contact Us</a></li>
</ul>